...
class Visualizer(object):
    ...
    def _init_map(self):
        ...
        nrows = int(self.field.height / self.grid_size)
        ncols = int(self.field.width / self.grid_size)
    ...
    def _handle_mouse_click(self, event):
        ...
        ncol = int((event.pos[0] - self.field.left) / self.grid_size)
        nrow = int((event.pos[1] - self.field.top) / self.grid_size)
        coord = (nrow, ncol)
    ...
    def _draw_grid(self, field):
        ...
        nrows = int(field.height / self.grid_size)
        ncols = int(field.width / self.grid_size)
    ...
    def _draw_map(self, field, blocked, start, goal, path):
        ...
        def _fill_spot(spot, color):
            nrow = spot[0]
            ncol = spot[1]
            pos_x = int(field.left + ncol * self.grid_size + self.grid_size / 2)
            pos_y = int(field.top + nrow * self.grid_size + self.grid_size / 2)
...
def draw_messages(screen, rect, message1, message2):
    draw_rimmed_box(screen, rect, (200, 20, 0), 4, Color('white'))
...
def draw_title(screen, rect):
    draw_rimmed_box(screen, rect, (40, 100, 60), 4, Color('gray'))
